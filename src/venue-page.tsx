    ...
    (canvasElement) => {
      ...
      if (props.url) {
        ...
        QRCode.toCanvas(
          ...
          [
            { data: "shc:/", mode: "byte" },
            { data: props.numeric!.join(""), mode: "numeric" },
          ],
          { errorCorrectionLevel: "L", scale: 20 },
          (error) => {
            canvasElement.style.width = "";
            canvasElement.style.height = "";
            if (error) console.error(error);
...
const App: React.FC<{
  ...
  const [verifierState, setVerifierState] = useState<VerifierState>(
    props.initialState
  );
  ...
  useEffect(() => {
    if (!verifierState?.siopRequest?.siopRequestQrCodeUrl) return;
  ...
  const displayRequest =
    verifierState?.siopRequest?.siopRequestQrCodeUrl && !displayResponse;

  let name, conclusion;
  if (displayResponse) {
    const fhirName =
      displayResponse?.idTokenVcs[0].vc.credentialSubject.fhirBundle.entry[0]
        .resource.name[0];
    name = fhirName?.given[0] + " " + fhirName?.family;
    conclusion = displayResponse?.idTokenVcs.map(
      (jwtPayload) =>
        jwtPayload.vc.credentialSubject.fhirBundle.entry[1].resource.conclusion
    );
  }
        ...
        {displayRequest && (
          <>
            <RS.Row>
              <RS.Col xs="12">
                <QrDisplay
                  url={verifierState.siopRequest.siopRequestQrCodeUrl}
                ></QrDisplay>
              </RS.Col>
            </RS.Row>
            <RS.Row>
              <RS.Col xs="12">
                <RS.Alert color="primary">Scan to continue</RS.Alert>
              </RS.Col>
            </RS.Row>
          </>
        )}
            ...
            <RS.Row>
              <RS.Col xs="12">
                ...
                >
                  ...
                  <RS.CardSubtitle className="text-muted">
                    Verified name: {JSON.stringify(name)}
                  </RS.CardSubtitle>
                  <RS.CardText style={{ fontFamily: "monospace" }}>
                    {" "}
                    {conclusion.join("; ")}
                  </RS.CardText>
                </RS.Card>
...
export default async function main() {
  ...
  ReactDOM.render(<App initialState={state} />, document.getElementById("app"));
}
