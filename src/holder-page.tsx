...
type UiEvent =
  | { type: "save-ui-state"; newState: UiState }
  | { type: "toggle-editing-config" }
  | { type: "begin-qr-presentation"; vc: HolderState["vcStore"][number] }
  | { type: "end-qr-presentation" }
  | { type: "open-scanner"; label: string }
  | { type: "close-scanner" }
  | { type: "scan-barcode" };
...
const App: React.FC<AppProps> = (props) => {
  const [holderState, setHolderState] = useState<HolderState>(
    props.initialHolderState
  );
  ...
  const issuerInteractions = holderState.interactions
    .filter((i) => i.siopPartnerRole === "issuer")
    .slice(-1);
  const verifierInteractions = holderState.interactions
    .filter((i) => i.siopPartnerRole === "verifier")
    .slice(-1);
  const siopAtNeedQr = issuerInteractions
    .concat(verifierInteractions)
    .filter((i) => i.status === "need-qrcode")
    .slice(-1);

  useEffect(() => {
    holderState.interactions
      .filter((i) => i.status === "need-redirect")
      .forEach((i) => {
        const redirectUrl =
          i.siopRequest.client_id +
          "#" +
          qs.encode(i.siopResponse.formPostBody);
    ...
    <div style={{ paddingTop: "5em" }}>
        ...
        <RS.Container>
          <NavbarBrand style={{ marginRight: "2em" }} href="/">
            <img
              ...
              style={{ maxHeight: "1em", maxWidth: "1em", marginRight: "10px" }}
              src={logo}
            />
            Health Wallet Demo
          </NavbarBrand>
          ...
          <Collapse navbar={true} isOpen={isOpen}>
            <Nav navbar={true}>
              ...
              <NavLink href="#" onClick={connectTo("verifier")}>
                {" "}
                Open Employer Portal
              </NavLink>
              <NavLink
                href="#config"
                onClick={(e) => dispatch({ type: "toggle-editing-config" })}
              >
                {" "}
                Edit Config
              </NavLink>
              <NavLink
                target="_blank"
                href="https://github.com/microsoft-healthcare-madison/health-wallet-demo"
              >
                Source on GitHub
              </NavLink>
            </Nav>
      ...
      {siopAtNeedQr.length > 0 && (
        <SiopRequestReceiver
          ...
          startUrl={
            siopAtNeedQr[0].siopPartnerRole === "issuer"
              ? uiState.issuer.issuerStartUrl
              : uiState.verifier.verifierStartUrl
          }
        />
      )}

      {uiState.editingConfig && (
        <ConfigEditModal
          uiState={uiState}
      ...
      {uiState.presentingQr?.active && (
        <QRPresentationModal
          healthCard={uiState.presentingQr.vcToPresent}
          dispatch={dispatch}
        />
      )}

      <SiopApprovalModal
        {...parseSiopApprovalProps(holderState, dispatchToHolder)}
      />
          ...
          <RS.Col xs="12">
            ...
            <Card style={{ padding: ".5em" }}>
              ...
              <CardSubtitle className="text-muted">
                Your browser's dev tools will show details on current page state
                + events{" "}
              </CardSubtitle>
            </Card>
          </RS.Col>
...
export default async function main() {
  const simulatedBarcodeScan = !!window.location.search.match(
    /simulate-barcode/
  );
